			...
			<div class="container-fluid">
                ...
                <div class="row page-titles mx-0">
                    <div class="col-sm-6 p-md-0">
                        <div class="welcome-text">
                            ...
                            <span>Data Riwayat Pinjaman</span>
                        </div>
                    </div>
                ...
                <div class="row">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    ...
                                    <table id="example3" class="display" style="min-width: 845px">
                                        ...
                                        <th class="text-center">Dana Yang Di terima 10%</th>
                                        <th class="text-center">Status</th>
                                        <th class="text-center">Foto Ditransfer </th>
                                        ...
                                        <?php
                  $no = 1;
                  $pinjaman = mysqli_query($koneksi, "SELECT * FROM pinjaman WHERE id_pinjaman ;");
                  while ($p = mysqli_fetch_array($pinjaman)) {
                    ...
                    $bunga = $p['jml_pinjaman']/10;
                    $hasil = $p['jml_pinjaman']-$bunga;
      
                   ?>	
                                            <tr>
                                        <td class="text-center"><?php echo $no++;?></td>
                                        <td class="text-center"><?php echo addslashes($p['no_trx']);?></td>
                                        <td class="text-center"><?php echo addslashes($p['tanggal_create']);?></td>
                                        <td class="text-center"><?php echo addslashes($u['nama_lengkap']);?></td>
                                        <td class="text-center">Rp.<?php echo addslashes(number_format($p['jml_pinjaman']));?></td>
                                        <td class="text-center">Rp.<?php echo addslashes(number_format($hasil));?></td>
                                        <td class="text-center"><a href="" class="badge badge-<?php echo addslashes($label);?> mr-5 mb-5"><?php echo addslashes($p['status']);?></a></td>
                                        <td class="text-center">
                                             <img class="img-avatar img-avatar48" width="50" src="../<?php echo $p['photo_bukti'];?>" alt="">
                                        </td>   
                                        <td class="text-center">
                                        <?php
                                    if(($p['status']=="Pending") or ($p['status']=="Ditolak"))
                                    {
                                        error_reporting(0);
                                        echo '
                                        <button  class="btn btn-primary shadow btn-xs sharp mr-1, open_modal" data-toggle="tooltip" id="'.$p[id_pinjaman].'" title="Upload Bukti TRansfer: '.$u[full_name].'">
                                        <i class="fa fa-pencil"></i>
                                    </button>';
                                }
                                      ?> 
                                     <?php
                                    if(($p['status']=="Ditransfer") or ($p['status']=="Ditransfer"))
                                    {
                                        error_reporting(0);
                                        echo '
                                        <button  class="btn btn-primary shadow btn-xs sharp mr-1, open_modal3" data-toggle="tooltip" id="'.$p[id_pinjaman].'" title="Upload Bukti TRansfer: '.$u[full_name].'">
                                        <i class="fa fa-pencil"></i>
                                    </button>';
                                }
                                      ?> 
                                        <!-- <a type="button" class="btn btn-circle btn-alt-danger mr-5 mb-5" data-toggle="tooltip" href="hapus/hsiswa.php?act=hapus&id_siswa=<?php echo addslashes($p['id_siswa']);?>&namafile=<?php echo addslashes($res['idsr_foto']);?>"title="Delete">
                                        <i class="fa fa-trash"></i>
                                    </a> -->
                                       

                                        <?php
                                    if(($p['status']=="Dikonfirmasi") or ($p['status']=="Dikonfirmasi"))
                                    {
                                        error_reporting(0);
                                        echo '
                                        <button  class="btn btn-primary shadow btn-xs sharp mr-1, open_modal2" data-toggle="tooltip" id="'.$p[id_pinjaman].'" title="Upload Bukti TRansfer: '.$u[full_name].'">
...
<script type="text/javascript">
 $(document).ready(function () {
   $(".open_modal2").click(function(e) {
       var m = $(this).attr("id");
		   $.ajax({
				   url: "edit/editpinjamanbukti.php",
				   type: "GET",
				   data : {id_pinjaman: m,},
				   success: function (ajaxData){
				   $("#ModalEdit2").html(ajaxData);
				   $("#ModalEdit2").modal('show',{backdrop: 'true'});
			   }
			});
        });
    });
...
<script type="text/javascript">
 $(document).ready(function () {
   $(".open_modal3").click(function(e) {
       var m = $(this).attr("id");
		   $.ajax({
				   url: "edit/editpinjamanbukti2.php",
				   type: "GET",
				   data : {id_pinjaman: m,},
				   success: function (ajaxData){
				   $("#ModalEdit3").html(ajaxData);
				   $("#ModalEdit3").modal('show',{backdrop: 'true'});
			   }
			});
        });
    });
