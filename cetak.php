        ...
        <div class="content-body">
							...
							<div class="modal-body">
								<form action="admin" method="POST" enctype='multipart/form-data'>
									<div class="form-group">
                                        ...
                                        <input type="hidden" name="hak_akses" value="admin" class="form-control">
									</div>
									<div class="form-group">
                ...
                <div class="row page-titles mx-0">
                    <div class="col-sm-6 p-md-0">
                        <div class="welcome-text">
                            ...
                            <span>Data Laporan Pinjaman</span>
                        </div>
                    </div>
            ...
            <form  method="get">
                <div class="form-group row">
                    ...
                    <div class="col-lg-3">
                        <label for="name">Filter by Nama</label>
                        <select name="id_user" id="name" class="mr-sm-2 default-select form-control">
                            <option selected="true" value="">Pilih Nama Karyawan</option>
                            ...
                            <?php 
                              ...
                              while ($p = mysqli_fetch_array($pinjaman)) {
                                $id_user = $p['id_user'];
                                $user = mysqli_query($koneksi, "SELECT nama_lengkap FROM user WHERE id_user = $id_user");
                                $u = mysqli_fetch_assoc($user);  
								
									?>	
									<option value="<?php echo $p['id_user']; ?>"><?php echo $u['nama_lengkap'] ?></option>
									<?php 
								}
								?>
                            </select>
                    ...
                    <!-- <div class="col-lg-2">
                        <label for="month">Filter by Bulan</label>
                        <select name="bln" id="bln" class="mr-sm-2 default-select form-control">
                            <option selected="true" value="">Pilih Nama Karyawan</option>
                            <option selected="true" value="">Semua</option>
                            <option  value="01">January</option>
                            <option  selected="true"  value="02">February</option>
                    ...
                    <!-- <div class="col-lg-2">
                        <label for="year">Filter by Tahun</label>
                    ...
                    <div class="col-lg-2">
                    <label for="year">Cari</label>
                        <button type="submit" class="btn btn-round btn-info waves-effect form-control">
					...
					<div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Laporan Dana Talangan Selesai</h4>
                                <!-- <button type="button" class="btn btn-primary mb-2" data-toggle="modal" data-target="#addProjectSidebar">Tambah Admin</button> -->
                                <button type="button" class="btn btn-primary mb-2" data-toggle="block-option" data-action="exportExcel" onclick="saveAsExcel('example3', 'laporan.xls')">
...
$result = mysqli_query($mysqli, "SELECT * FROM user where hak_akses = 'admin'");
$no=1;
?>
                                    <table id="example3" border="2px" class="display" style="min-width: 845px">
                                        <thead>
                                            <tr>
                                <th>Tgl. Lunas</th>
                                <th>Nama</th>
                                ...
                                <th>Status</th>
                                        ...
                                        <tbody>
                                        <?php
                        $query  = "SELECT * FROM pinjaman WHERE  status = 'Selesai' ";
                        $pinjaman = mysqli_query($mysqli, $query);
                  $no = 1;
                  if(addslashes(isset($_GET['id_user']))){
                    $id_user = addslashes($_GET['id_user']);
                  $pinjaman = mysqli_query($koneksi, "SELECT * FROM pinjaman WHERE id_user LIKE '%$id_user%'");
                }
                  while ($p = mysqli_fetch_array($pinjaman)) {
                    $id_user = $p['id_user'];
                    $user = mysqli_query($koneksi, "SELECT nama_lengkap FROM user WHERE id_user = $id_user");
                   ...
                   ?>	
                        <tr>
                                <td><?php echo addslashes($p['tanggal_update']);?></td>
                                <td><?php echo addslashes($u['nama_lengkap']);?></td>
                                <td><?php echo addslashes($p['jml_pinjaman']);?>	</td>
                                <td><a href="" class="badge badge-<?php echo addslashes($label);?> mr-5 mb-5"><?php echo addslashes($p['status']);?></a></td>
                        </tr>
                ...
                ?>
                 <?php
                  $query  = "SELECT sum(jml_pinjaman) as total FROM pinjaman WHERE  status = 'Selesai' ";
                  $pinjaman = mysqli_query($mysqli, $query);
                  $query2  = "SELECT * FROM pinjaman WHERE  status = 'Selesai' ";
                  $pinjaman2 = mysqli_query($mysqli, $query2);
                  if(addslashes(isset($_GET['id_user']))){
                    $id_user = addslashes($_GET['id_user']);
                 $pinjaman = mysqli_query($koneksi, "SELECT sum(jml_pinjaman) as total FROM pinjaman WHERE id_user LIKE '%$id_user%' ");
                 $pinjaman2 = mysqli_query($koneksi, "SELECT * FROM pinjaman WHERE id_user LIKE '%$id_user%' ");
                  }
                 $p2 = mysqli_fetch_assoc($pinjaman);
                 $Selesai = mysqli_num_rows($pinjaman2); {
                ...
                ?>	
                 <tr class="table-primary">
                                <td><b>JUMLAH DATA = <?php echo $Selesai;?></b></td>
                                <td><b>JUMLAH</b></td>
                                <td><b>Rp.<?php echo addslashes(number_format($p2['total']));?>,-</b></td>
                            </tr>
                            <?php
                  }
                  ?>
